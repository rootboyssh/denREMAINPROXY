...
get_user_input() {
    ...
    echo "Введите ваш пул IPv6 адресов (например, 2a06:f901:4001:0100::/64), или оставьте пустым для автодетекции:"
    read ipv6_pool
...
# Если пользователь указал пул IPv6, используем его для генерации адресов
if [ -n "\$ipv6_pool" ]; then
  rnd_pool_ip () {
...
# Генерация случайных IPv6 адресов для прокси и запись в файл
count=1
...
echo "net.ipv4.route.min_adv_mss = 1200" >> /etc/sysctl.conf
