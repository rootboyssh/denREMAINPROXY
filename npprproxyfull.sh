...
# Функция интерактивного ввода параметров (без запроса IPv6-пула)
get_user_input() {
...
is_auth_used() {
    # Если включена рандомная авторизация, то auth используется
    if [ "$use_random_auth" = true ]; then
        return 0
    fi
    # Если логин и пароль заданы, auth используется
    if [ -n "$user" ] && [ -n "$password" ]; then
        return 0
    fi
    # Иначе авторизация не используется
    return 1
}
