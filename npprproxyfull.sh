#!/bin/bash
# Исправленный скрипт для автоматического определения IPv6-пула и создания прокси
...
# Функция для автоматического определения доступного IPv6-пула
get_available_ipv6_pool() {
    # Определяем интерфейс, кроме lo и виртуальных
    local iface
    iface=$(ip -br l | awk '$1 !~ /lo|vir|wl|@NONE/ { print $1; exit }')
    if [ -z "$iface" ]; then
        log_err_and_exit "Ошибка: не найден подходящий сетевой интерфейс"
    fi
    # Просматриваем все глобальные IPv6 адреса с маской, отличной от /128
    local pool=""
...
# Функция проверки базового формата IPv4 (для backconnect)
is_valid_ip() {
    if [[ "$1" =~ ^(([0-9]{1,3}\.){3}[0-9]{1,3})$ ]]; then return 0; else return 1; fi
}

# Интерактивный ввод параметров (без запроса IPv6-пула – он определяется автоматически)
get_user_input() {
...
# Опции командной строки оставляем без изменений
usage() { echo "Usage: $0 [options]" 1>&2; exit 1; }
...
# Функции для проверки, установки и управления процессами остаются без значительных изменений
...
close_ufw_backconnect_ports() {
  ...
  ufw delete allow "$first_opened_port":"$last_opened_port"/tcp >> "$script_log_file" 2>&1
  ufw delete allow "$first_opened_port":"$last_opened_port"/udp >> "$script_log_file" 2>&1
...
# Если вывод информации запрошен, выводим и выходим
...
# Первоначальные настройки системы (с резервной копией sysctl.conf)
...
# В конце выводим список прокси на экран
echo -e "\nСформированные прокси (с учётом IPv6 пула):"
