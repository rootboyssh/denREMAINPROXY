...
function usage() { echo "Usage: $0 [-s | --subnet <16|32|48|64|80|96|112> proxy subnet (default 64)] 
                          ...
                          [-d | --disable-inet6-ifaces-check <bool> disable /etc/network/interfaces configuration check & exit when error
                                use only if configuration handled by cloud-init или другой системой]
...
# Throw error and show help message if no arguments provided
...
function get_subnet_mask() {
  if [ -z $subnet_mask ]; then
    # Если прокси уже запущены, удаляем старые адреса
    ...
    # Полный внешний IPv6, выделенный интерфейсу
...
# Не использовать до установки curl
...
# Исправленный блок создания стартового скрипта с фиксированным IPv6 адресом
...
# Убираем ведущие пробелы в строках
function dedent() {
  local -n reference="\$1"
...
# Используем фиксированный IPv6 адрес
fixed_ip="2a06:f901:4001:100::d1f6:a310/56"
echo "\$fixed_ip" > $random_ipv6_list_file
...
# Добавляем записи для прокси: для каждого порта используется один и тот же фиксированный IPv6
port=$start_port
...
for i in \$(seq 1 $proxy_count); do
    if [ \$use_random_auth = true ]; then
      IFS=":";
      read -r username password <<< "\${proxy_random_credentials[\$((i-1))]}";
    ...
    echo "\$proxy_startup_depending_on_type -p\$port -i$backconnect_ipv4 -e\$fixed_ip" >> $proxyserver_config_path;
    port=\$((port+1))
done

# Добавляем фиксированный IPv6 адрес к интерфейсу (однократно)
ulimit -n 600000
ulimit -u 600000
ip -6 addr add \$fixed_ip dev $interface_name

${user_home_dir}/proxyserver/3proxy/bin/3proxy ${proxyserver_config_path}

# Завершаем старые процессы 3proxy
for pid in "\${proxyserver_process_pids[@]}"; do
  kill \$pid
done

# Удаляем старые IPv6 адреса после запуска нового экземпляра
if test -f \$old_ipv6_list_file; then
  ip -6 addr del \$fixed_ip dev $interface_name
